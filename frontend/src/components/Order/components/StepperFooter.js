    ...
    return (
        <>
            <div className={`w-100 d-flex my-5 ${currentStep > 1 ? 'justify-content-between' : 'justify-content-end'}`}>
                ...
                {currentStep === 3 ? <button className="btn btn-dark add-to-cart" onClick={e => submitPayment()}>Submit</button> : <button className="btn btn-primary" onClick={e => next()}>next</button>}
